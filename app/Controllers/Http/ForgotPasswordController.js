...
class ForgotPasswordController {
  async store ({ request, response }) {
    try {
      ...
      await Mail.send(
        ...
        { email, link: `${request.input('redirect_url')}?token=${user.token}` },
        message => {
          message
            .to(user.email)
  ...
  async update ({ request, response }) {
    try {
      ...
      if (user.token !== token) {
        throw new Error('Password reset token invalid')
      }
      ...
      if (tokenExpired) {
        return response.status(401).send({
          error: { message: 'O token de recuperação expirou, gere um novo.' }
        })
      }
